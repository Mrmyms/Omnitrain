...
std_X_w = None
conf_w = None
map_path_w = "data/maps/example_map"
base_mask_w = None
...
def evaluate_agent(model_state_dict):
    ...
    obs, _, done, _ = env.reset(np.array([[conf_w.sx, conf_w.sy, conf_w.stheta]]))
    ...
    # Fitness Function = Survival Time + Distance Bonus
    fitness = steps + (distance_traveled * 2.0)
    return fitness

def run_evolution():
    print("--- STARTING EVOLUTIONARY STRATEGY RL PIPELINE ---")
    
    # 1. Load the "Adam" pre-trained Reflex Arc
    hidden_R = 100
    n_sen_R = 50
    n_pro_R = 25
    n_hdr_R = 25
    ...
    if os.path.exists("data/f110_reflex_100.pt"):
        print("Loaded Adam from pre-trained Reflex Arc model.")
        adam.load_state_dict(torch.load("data/f110_reflex_100.pt", map_location='cpu'))
    else:
        print("Pre-trained model not found! Evolving from purely random weights (This will take forever).")
