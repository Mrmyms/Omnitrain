...
void IRAM_ATTR OmniEngineGRU::Step(const float* sensors, float* out_action) {
    if (!is_loaded_) {
    ...
    matmul(w_ih_, b_ih_, sensors, gates_ih_, 3 * d_model_, input_dim_);
    matmul(w_hh_, b_hh_, h_state_, gates_hh_, 3 * d_model_, d_model_);

    // Apply GRU gating (r, z, n)
    for (uint32_t i = 0; i < d_model_; ++i) {
        float r_gate = sigmoid(gates_ih_[i] + gates_hh_[i]);
        float z_gate = sigmoid(gates_ih_[i + d_model_] + gates_hh_[i + d_model_]);
        float n_gate = std::tanh(gates_ih_[i + 2*d_model_] + r_gate * gates_hh_[i + 2*d_model_]);
